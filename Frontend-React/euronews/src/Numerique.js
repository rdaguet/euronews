...
const Politique = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3001/api/articles')
            .then(response => {
                // Filtrer les articles avec le tag "numérique"
                const filteredArticles = response.data.filter(article => article.tags && article.tags.includes("numérique"));
    ...
    return (
        <div>
            ...
            <div className="bannernumerique">
                <h1>NUMERIQUE</h1>
            </div>

            <h1 id='Articles'>Numérique</h1>
            <div>
